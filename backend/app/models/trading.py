...
class AIModelMetadata(Base):
    ...
    champion: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    approval_status: Mapped[str] = mapped_column(String(24), default="pending", index=True)
    approved_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    approved_by: Mapped[str | None] = mapped_column(String(255), nullable=True)
    challenger_of_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    lifecycle_metadata: Mapped[dict] = mapped_column(JSON, default=dict)
    model_drift: Mapped[dict] = mapped_column(JSON, default=dict)
    ...
    retrain_interval_hours: Mapped[int | None] = mapped_column(Integer, nullable=True)
    next_retrain_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
